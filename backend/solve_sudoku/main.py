...
def solve_sudoku(request):
    ...
    solved_matrix = merge_matrix(blocks, solve(np.array(values), np.array(blocks)).tolist())
    print(matrix, solved_matrix)
    return json.dumps(solved_matrix), 200, headers
...
def solve(init, blocks, generation=False):
    """
